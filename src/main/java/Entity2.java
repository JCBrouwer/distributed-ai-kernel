...
public class Entity2 {
    ...
    private void perform() {
        ...
        while (it > 0) {
            ...
            if (it % 100 == 0) {
                // System.out.println(LSQRS(x,y,new utils.LinearRegression(x,y).slope(),new
                // utils.LinearRegression(x,y).intercept()) /
                // LSQRS(x,y,average.X(),average.Y()));
                // series.add(N-it,LSQRS(x,y,new LinearRegression(x,y).slope(),new
                // LinearRegression(x,y).intercept()) / LSQRS(x,y,average.X(),average.Y()));
